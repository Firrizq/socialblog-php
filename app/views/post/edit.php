...
<form id="editPostForm" action="<?= BASEURL ?>/post/edit/<?= $data['post']['id'] ?>" method="POST" class="flex flex-col w-full min-h-screen">
    ...
    <div class="sticky top-16 z-40 bg-surface/95 backdrop-blur-md border-b border-outline-variant/30 py-3 px-4 sm:px-6 mb-8 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 min-w-0 flex-1 overflow-x-auto no-scrollbar py-0.5">
            <a href="<?= ($data['post']['status'] ?? '') === 'draft' ? BASEURL . '/profile' : BASEURL . '/post/detail/' . $data['post']['id'] ?>" class="w-8 h-8 rounded-full hover:bg-surface-container text-on-surface-variant hover:text-on-surface flex items-center justify-center transition-colors shrink-0 mr-1" title="Cancel & Return">
        ...
        <div class="flex items-center gap-3 shrink-0">
            <a href="<?= ($data['post']['status'] ?? '') === 'draft' ? BASEURL . '/profile' : BASEURL . '/post/detail/' . $data['post']['id'] ?>" class="text-sm font-medium text-on-surface-variant hover:text-on-surface transition-colors px-2 py-1.5 rounded-lg hover:bg-surface-container">Cancel</a>
